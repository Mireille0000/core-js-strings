...
function isPalindrome(str) {
  const strToLowerCase = str.replace(/[\s.,?!]/g, '').toLowerCase();
  let result = '';

  for (let i = 0; i < strToLowerCase.length; i += 1) {
    result += strToLowerCase[strToLowerCase.length - 1 - i];
  }

  return result === strToLowerCase;
}
...
function findLongestWord(sentence) {
  ...
  const maxLength = Math.max.apply(null, result);
  const theLongestWordIndex = result.indexOf(maxLength);

  return words[theLongestWordIndex];
}
